...
interface EnhancedParticipantJoinProps {
  trigger?: React.ReactNode
  className?: string
  onJoinSuccess?: (sessionId: string, roomCode: string, participantName: string) => void
}

export function EnhancedParticipantJoin({
  trigger,
  className,
  onJoinSuccess
}: EnhancedParticipantJoinProps) {
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      ...
      const activeViewersQuery = query(
        collection(db, "liveDrawSessions", sessionPreview.sessionId, "viewers"),
        where("isActive", "==", true)
      )
      ...
      const sessionDoc = await getDoc(doc(db, "liveDrawSessions", sessionPreview.sessionId))
      ...
      // Redirect to live viewer with viewerId for tracking
      router.push(`/live/${sessionPreview.sessionId}?name=${encodeURIComponent(participantName)}&viewerId=${viewerId}&platform=${platform}`)
